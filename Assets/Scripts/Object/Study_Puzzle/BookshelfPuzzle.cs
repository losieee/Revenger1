...
public class BookshelfPuzzle : MonoBehaviour
{
    ...
    // 각각 달성 여부
    private bool normalCleared = false;
    private bool secretCleared = false;
    ...
    public void ValidateIfFull()
    {
        ...
        // 각각 판정
        bool gotSecret = CheckIds(secretIds);
        bool gotNormal = CheckAnswer();

        // 비밀 정답 처리
        if (gotSecret && !secretCleared)
        {
            secretCleared = true;
            RotateSecretTarget();
    ...
    public void Validate()
    {
        ...
        bool gotSecret = CheckIds(secretIds);                 // 비밀 정답
        bool gotNormal = CheckIds(answerIds) || CheckTypes(); // 일반 정답
    ...
    void MarkSolved()
    {
        ...
        // lockOnSuccess가 true여도 두 개 모두 충족했을 때만 잠금이 실행됨
